...
public class LocalisationTranslator {
  ...
  public static String getEvent(String event) {
    return switch (event) {
      ...
      case LifecycleResource.EVENT_EXEMPT:
        yield LocalisationResource.EVENT_STATUS_BILLABLE_EXEMPT_KEY;
    ...
    return switch (eventStatusLocalisedKey) {
      ...
      case LocalisationResource.EVENT_STATUS_BILLABLE_ISSUE_KEY:
        yield Rdf.literalOf(LifecycleResource.EVENT_ACCRUAL + ";" + LifecycleResource.EVENT_INCIDENT_REPORT)
            .getQueryString();
      case LocalisationResource.EVENT_STATUS_BILLABLE_EXEMPT_KEY:
        yield Rdf.literalOf(LifecycleResource.EVENT_EXEMPT).getQueryString();
  ...
  public static String getScheduleTypeFromRecurrence(String recurrence) {
    return switch (recurrence) {
      case null -> LocalisationResource.PERPETUAL_SERVICE_KEY;
      case "" -> LocalisationResource.PERPETUAL_SERVICE_KEY;
      case LifecycleResource.RECURRENCE_FIXED_DATE_TASK -> LocalisationResource.FIXED_DATE_SERVICE_KEY;
      case LifecycleResource.RECURRENCE_DAILY_TASK -> LocalisationResource.SINGLE_SERVICE_KEY;
      case LifecycleResource.RECURRENCE_ALT_DAY_TASK -> LocalisationResource.ALTERNATE_DAY_SERVICE_KEY;
      default -> LocalisationResource.REGULAR_SERVICE_KEY;
    };
  }
}
